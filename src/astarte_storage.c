...
{
    if (!data) {
        // Delete returns ASTARTE_STORAGE_ERR_OK when property is not present
    ...
    const char *interface_name, const char *path, const void *data, size_t data_len, bool *res)
{
    ...
    *res = false;
    ...
    esp_err_t esp_err = astarte_nvs_key_value_get_blob(handle.nvs_handle, key, NULL, &tmp_data_len);
    if ((esp_err != ESP_ERR_NVS_NOT_FOUND) && (esp_err != ESP_OK)) {
        free(key);
        return ASTARTE_STORAGE_ERR_INTERNAL;
    }
    if ((esp_err == ESP_ERR_NVS_NOT_FOUND) || (tmp_data_len != data_len)) {
        free(key);
        return ASTARTE_STORAGE_ERR_OK;
    }

    // Allocate temporary data
    char *tmp_data = calloc(tmp_data_len, sizeof(char));
    if (!tmp_data) {
        ESP_LOGE(TAG, "Out of memory %s: %d", __FILE__, __LINE__);
        free(key);
        return ASTARTE_STORAGE_ERR_INTERNAL;
    ...
    // Get stored data
    astarte_nvs_key_value_get_blob(handle.nvs_handle, key, tmp_data, &tmp_data_len);
    free(key);
    if (esp_err != ESP_OK) {
        free(tmp_data);
        return ASTARTE_STORAGE_ERR_INTERNAL;
    ...
    // Check stored data
    if (memcmp(tmp_data, data, data_len) == 0) {
        *res = true;
    }
...
{
    esp_err_t esp_err
        = astarte_nvs_key_value_iterator_peak(&iterator->nvs_key_value_iterator, has_next);
    if (esp_err != ESP_OK) {
        return ASTARTE_STORAGE_ERR_INTERNAL;
    }
    return ASTARTE_STORAGE_ERR_OK;
