...
esp_err_t astarte_nvs_key_value_set_i32(nvs_handle_t handle, const char *key, int32_t value)
{
    ...
    esp_err = nvs_set_i32(handle, value_entry_name, value);
    if (esp_err != ESP_OK) {
        ESP_LOGE(TAG, "Error storing the value.");
        nvs_erase_key(handle, key_entry_name);
        return esp_err;
...
esp_err_t astarte_nvs_key_value_get_i32(nvs_handle_t handle, const char *key, int32_t *out_value)
{
    ...
    esp_err = nvs_get_i32(handle, value_entry_name, out_value);
    if (esp_err != ESP_OK) {
        ESP_LOGE(TAG, "Error fetching the value.");
    }
    return esp_err;
...
esp_err_t astarte_nvs_key_value_erase_key_i32(nvs_handle_t handle, const char *key)
{
    ...
    for (uint64_t i = key_store_index + 2; i < next_store_index; i += 2) {
        ...
        int32_t tmp_value = 0;
        esp_err = nvs_get_i32(handle, tmp_value_entry_name, &tmp_value);
        if (esp_err != ESP_OK) {
            ESP_LOGE(TAG, "Error fetching value from nvs during erase operation.");
            free(tmp_key);
            return esp_err;
        ...
        esp_err = nvs_set_i32(handle, new_value_entry_name, tmp_value);
        if (esp_err != ESP_OK) {
            ESP_LOGE(TAG, "Error storing the value.");
            return esp_err;
        }
    ...
    char *out_key, size_t *out_key_len, int32_t *out_value)
{
    ...
    if (out_value) {
        ...
        esp_err = nvs_get_i32(iterator->handle, value_entry_name, out_value);
        if (esp_err != ESP_OK) {
            ESP_LOGE(TAG, "Error fetching key from nvs during iteration.");
            return esp_err;
        }
