...
/**
 * @brief Sets a 32 bits integer for given key.
 *
 * @details Behaves as similar as possible to the nvs_set_i32 function found in nvs_flash.
 *
 * @param[in] handle Handle obtained from nvs_open function. Read-only handles cannot be used.
 * @param[in] key Key name. Maximum length is 4000 bytes, including the terminating char.
 * @param[in] value Buffer containing the value to set.
 * @return An ESP_OK when operation has been successful, an error code otherwise.
 */
esp_err_t astarte_nvs_key_value_set_i32(nvs_handle_t handle, const char *key, int32_t value);
...
/**
 * @brief Gets the 32 bits integer for the given key.
 *
 * @details Behaves as similar as possible to the nvs_get_i32 function found in nvs_flash.
 *
 * @param[in] handle Handle obtained from nvs_open function.
 * @param[in] key Key name. Maximum length is 4000 bytes, including the terminating char.
 ...
 * @return An ESP_OK when operation has been successful, an error code otherwise.
 */
esp_err_t astarte_nvs_key_value_get_i32(nvs_handle_t handle, const char *key, int32_t *out_value);
 ...
 * @return An ESP_OK when operation has been successful, an error code otherwise.
 */
esp_err_t astarte_nvs_key_value_erase_key_blob(nvs_handle_t handle, const char *key);
 ...
 * @return An ESP_OK when operation has been successful, an error code otherwise.
 */
esp_err_t astarte_nvs_key_value_erase_key_i32(nvs_handle_t handle, const char *key);
 ...
 * @return An ESP_OK when operation has been successful, an error code otherwise.
 */
esp_err_t astarte_nvs_key_value_iterator_get_element_blob(
    astarte_nvs_key_value_iterator_t *iterator, char *out_key, size_t *out_key_len, void *out_value,
    size_t *out_value_len);

/**
 * @brief Gets the entry currently pointed to by the iterator.
 *
 * @note This should be used only for an iterator that has been initialized with the i32 type.
 *
 * @details Can be used in a similar manner as any other getter function from nvs.
 * The function can be called first with the buffer pointers set to NULL to obtained the required
 * buffer sizes. Then called a second time with a pair of pre-allocated buffers.
 ...
 * @return An ESP_OK when operation has been successful, an error code otherwise.
 */
esp_err_t astarte_nvs_key_value_iterator_get_element_i32(astarte_nvs_key_value_iterator_t *iterator,
    char *out_key, size_t *out_key_len, int32_t *out_value);
